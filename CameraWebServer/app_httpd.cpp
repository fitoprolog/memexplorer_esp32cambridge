...
static esp_err_t robot_codes_handler(httpd_req_t *req)
{
    char *buf = NULL;
    char data[32];
    char tx_buf[48];
    ...
    int command = parse_get_var(buf, "command", 0);
    int value = parse_get_var(buf, "value", 0);
    int duration = parse_get_var(buf, "duration", 0);
    free(buf);

    int tx_len = snprintf(tx_buf, sizeof(tx_buf), "%d %d %d\n", command, value, duration);
    if (tx_len > 0) {
        uart_write_bytes(UART_NUM_0, tx_buf, tx_len);
    }

    // Get the number of bytes waiting in the UART buffer
    ...
    if (length > 0) {
        ...
        // Read data from UART with a timeout of 20ms
        int read_bytes = uart_read_bytes(UART_NUM_0, (uint8_t *)data, length, 20 / portTICK_PERIOD_MS);

        if (read_bytes > 0) {
            httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
            httpd_resp_set_type(req, "application/octet-stream");
            return httpd_resp_send(req, data, read_bytes);
        }
    }
